...
(function () {
    ...
    //to feature test w/o causing perf issues.
        isPS3 = isBrowser && navigator.platform === 'PLAYSTATION 3',
        readyRegExp = isPS3 ? /^complete$/ : /^(complete|loaded)$/,
    ...
    req.checkLoaded = function (contextName) {
        ...
        if (stillLoading) {
            ...
            context.isCheckLoaded = false;
            if (!isPS3 && (isBrowser || isWebWorker)) {
                setTimeout(function () {
    ...
    req.execCb = function (name, cb, args) {
        ...
        try {
            ...
            log.info('Hacky error complaint: ', e)
            return null;
        }
    };
